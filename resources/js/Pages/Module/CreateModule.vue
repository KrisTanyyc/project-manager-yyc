  ...
  <div class="flex flex-row h-full w-full">
    ...
    <div class="flex flex-col flex-auto bg-white w-4/5 h-screen">
      ...
      <div class="flex flex-col flex-auto h-4/5 w-full bg-white">
        ...
        >
          <div class="mt-5 ml-5">
            <span class="font-bold text-3xl">Create Module</span>
          </div>
          <div class="m-3 mt-10">
          ...
          <div class="pt-5 mr-10 flex flex-row justify-end">
            ...
            <a
              :href="'/projects/' + project_id +'?tab=2'"
  ...
  methods: {
    checkValidation() {
      ...
      if (!this.smodule.name) {
        this.errors["name"] = "Please enter the module name";
      }
    },

    createModule() {
      this.errors = [];
      var url = "/api/project/" + this.project_id + "/module/create",
        method = "post";
      fetch(url, {
        method: method,
        body: JSON.stringify({
        ...
        .then((response) => {
          if (response.success) {
            window.location.href = "/projects/" + this.project_id +"?tab=2";
          }
        })
        .catch((error) => {
