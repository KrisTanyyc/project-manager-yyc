  ...
  <div class="flex flex-row h-full w-full">
    ...
    <div class="flex flex-col flex-auto bg-white w-4/5 h-screen">
      ...
      <div class="flex flex-col flex-auto h-4/5 w-full bg-white">
        ...
        >
          <div class="mt-5 ml-5">
            <span class="font-bold text-3xl">Update Snapshot</span>
          </div>
          <div class="m-3 mt-10">
          ...
          <div v-if="'snapshot' in errors" class="flex justify-center mt-5">
...
export default defineComponent({
  ...
  methods: {
    checkValidation() {
      this.errors = [];
      if (this.snapshotName !== "" && this.snapshotName !== null) {
        this.updateSnapshot();
        this.isButtonClicked = true;
      }
      if (!this.snapshotName) {
        this.errors["snapshot"] = "Please enter the snapshot name";
      }
    },
        ...
        .then((response) => {
          if (response.success) {
            window.location.href =
              "/projects/" + this.project.id + "?tab=3";
          }
        })
        .catch((error) => console.log("Error updating snapshot: ", error));
    },
  },
});
</script>
